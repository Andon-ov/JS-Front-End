function solve(arr) {
    ...
    for (const allText of arr) {
        let result = JSON.parse(allText);
        for (let resultKey in result) {
            dict[resultKey] = result[resultKey];
        }
    }


    let sortable = Object.fromEntries(Object.entries(dict).sort(([a,], [b,]) => a.localeCompare(b)));

    for (const sortableKey in sortable) {
        console.log(`Term: ${sortableKey} => Definition: ${sortable[sortableKey]}`)
